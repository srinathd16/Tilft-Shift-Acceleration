    ...
    */
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            color = ((int) AA & 0xff) << 24 | (((int) RR) & 0xff) << 16 | (((int) GG) & 0xff) << 8 |
                    (((int) BB) & 0xff);
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            for (int i = 0; i < size; i = i + 16) {
                ...
                color = ((int) AA & 0xff) << 24 | ((res_R) & 0xff) << 16 | ((res_G) & 0xff) << 8 |
                        ((res_B) & 0xff);


                pixels[y * width + x] = color;
                new_pixels[(max_size_chk / 2 + y) * new_width + (max_size_chk / 2) + x] = color;
            }
